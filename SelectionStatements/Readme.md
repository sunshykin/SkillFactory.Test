...
Данная конструкция выполняет блок кода в зависимости от значения указанной переменной.
...
блоки кода для каждого проверяемого значения. При этом переменная должна быть иметь один из следующих типов: ```char```, ```bool```, ```string```, целочисленным типом
...
Данный оператор по окончанию выполнения своего блока кода отдает управление в указанный им блок ```case```, и выход из выражения уже будет зависеть от оператора в указанном блоке.

**4. Оператор выброса исключения — Оператор ```throw```.**

Данный оператор работает с исключениями, о которых будет рассказано позднее.

**5. Отсутствие оператора.**

Также, блок кода может быть не ограничен операторами перехода. В таком случае, при завершении кода блока будет выполняться код следующего по коду блока.
...
В данном примере через конструкцию ```switch``` устроено определение стоимости мороженного, в зависимости от его размера: маленькое - 25 рублей, среднее - на 25 дороже
(т.е. 50) и большое ещё на 30 дороже - 80 рублей. Когда выбран маленький размер, исполняется блок кода ```case IceCreamType.Small:``` и результат возвращается из функции.
Когда же размер мороженного - средний или большой, происходит выполнения кода в нужном блоке, а вместе с ним и выполнение кода для меньшего размера с помощью оператора
```goto case``` для большого мороженного и отсутствия оператора для среднего.
