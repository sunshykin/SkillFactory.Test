...
Также блок кода может быть не ограничен операторами перехода. В таком случае при завершении кода блока будет выполняться идущий следом блок.
...
Когда же размер мороженого - средний или большой, происходит выполнения кода в нужном блоке, а вместе с ним и выполнение кода для меньшего размера с помощью оператора
```goto case``` для большого мороженого и отсутствия оператора для среднего.
